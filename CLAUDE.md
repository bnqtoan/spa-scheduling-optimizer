...
- **7 tables** (`src/server/db/schema.ts`): technicians, skills, technician_skills (M2M), services (skillId required, durationMin, price VND), working_hours (weekday/startMin/endMin), time_off (date + optional startMin/endMin; null/null = whole day), bookings (code unique `SPA{yymmdd}-{seq}`, status scheduled|completed|cancelled).
...
- API routers each `export default` a Hono sub-app (technicians also exports `skillsRoute`); all mounted in `src/server/app.ts` at `/api/{technicians,skills,services,schedule,bookings,slots}`.
...
### Scope (PRD non-goals — do NOT add)
No auth/login, payment, SMS/email, Google Calendar sync, multi-branch, multi-tenant, online meeting, membership/loyalty. Roles: Admin (manages everything) and Customer (no account; books via the `/book` wizard).
